        ...
        <div class="row">
            ...
            <div class="col-md-12 mb-4">
                <nav class="navbar navbar-light bg-light" style="box-shadow: 0 5px 5px rgb(0 0 0 / 35%)">
                    <ul class="nav justify-content-end">
                        <li class="nav-item">
                            <a class=" btn btn-sm btn-outline-success"
                                href="{{ route('admin.add.boat') }}">{{ __('add_new') }}</a>
                        </li>
                    </ul>
                    <ul class="nav justify-content-end">
                        <li class="nav-item">
                            <form class="form-inline my-2 my-lg-0" action="{{ route('admin.search.boat') }}">
                                <input class="form-control mr-sm-2" type="text" name="search"
                                    value="{{ $search }}" placeholder="{{ __('search_boat') }}"
                                    aria-label="Search" />
                    ...
                    <ul class="nav justify-content-end">
                        <li class="nav-item">
                            <select name="orderby" class="form-control-sm" wire:model="sorting">
                                <option value="default" selected="selected">{{ __('default_sorting') }}</option>
                                <option value="date">{{ __('date') }}</option>
                                <option value="low">{{ __('low') }}</option>
                                <option value="hight">{{ __('hight') }}</option>
            ...
            <div class="col-md-12">
                    ...
                    <table class="table table-striped" style="box-shadow: 0 5px 5px rgb(0 0 0 / 35%)">
                        ...
                        <tbody>
                            @foreach ($boats as $boat)
                                <tr>
                                    <td width="100"><img class="card-img-top w-50 h-50 rounded-circle lightboxed"
                                            rel="group1"
                                            src="{{ asset('assets/images/boat') }}/{{ $boat->photo_path }}"
                                            data-link="{{ asset('assets/images/boat') }}/{{ $boat->photo_path }}"
                                            data-caption="{{ $boat->name }}"></td>
                                    <td>{{ $boat->name }}</td>
                                    ...
                                    <td>
                                        <div>
                                            <a href="#" onclick="deleteBoat({{ $boat->id }})"><span
                                                    class="btn btn-danger position-relative mr-2"
                                                    style="top:5px;">{{ __('Delete') }}</span></a>

                                            <a href="{{ route('admin.edit.boat', ['boat' => $boat->id]) }}"
                                                class="text-success"><span
                                                    class="btn btn-primary position-relative mr-2"
                                                    style="top:5px;">{{ __('Edit') }}</span></a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                ...
                @else
                    <div class="alert alert-danger mt-4" role="alert">
                        {{ __('no results found') }} <a href="{{ route('admin.boats') }}"
                            class="alert-link">{{ __('back to boat page') }}</a>.
                    </div>
                @endif
            </div>
        </div>
